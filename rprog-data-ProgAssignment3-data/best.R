best <- function (stat, outcome) {
    ...
    if(outcome == "heart attack") {
        ...
        attack <- data[keeps]
        attackState <- subset(attack,State == stat & Hospital.30.Day.Death..Mortality..Rates.from.Heart.Attack!= "Not Available")
        ...
        attack <- data[keeps]
        attackState <- subset(attack,State == stat & Hospital.30.Day.Death..Mortality..Rates.from.Heart.Failure!= "Not Available")
        attackState[,3] <- as.numeric(attackState[,3])
        attackState <- attackState[order(attackState[,3]),]
        min <- attackState[1,3]
        ...
        attack <- data[keeps]
        attackState <- subset(attack,State == stat & Hospital.30.Day.Death..Mortality..Rates.from.Pneumonia!= "Not Available")
        attackState[,3] <- as.numeric(attackState[,3])
        attackState <- attackState[order(attackState[,3]),]
        min <- attackState[1,3]
